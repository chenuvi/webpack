...
import { BehaviorSubject, interval, fromEvent, combineLatest } from 'rxjs'
...
const renderGame = (state: State) => {
  ;(app.innerHTML = `Score: ${state.score}, Level: ${state.level} <br/>`),
    state.letters.forEach(
      (l) => (app.innerHTML += '&nbsp'.repeat(l.yPos) + l.letter + '<br/>')
    ),
    (app.innerHTML +=
...
const renderGameOver = () => (app.innerHTML += '<br/>GAME OVER!')
const noop = () => {}
...
const keys$ = fromEvent(document, 'keydown').pipe(
  startWith({ key: '' }),
  // map((e: KeyboardEvent) => e.key) // TODO
  map((e: any) => e.key)
)
